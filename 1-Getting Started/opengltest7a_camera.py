...
Excercise:  Camera
            ...
            around the scene (we moved backwards a little). OpenGL by itself is not familiar with the concept
            of a camera, but we can try to simulate one by moving all objects in the scene in the reverse direction,
...
while True:
    ...
    #       our goal is to rotate the camera pointing at the center (via cameraTarget) by rotating around the Y axis (which stays at 0)
    radius = 10.0
