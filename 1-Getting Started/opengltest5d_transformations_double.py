...
Excercise: Try drawing a second container with another call to glDrawElements but place it at a
           different position using transformations only. 
           Make sure this second container is placed at the top-left of the window and instead of rotating
           scale it over time (using the sin function is useful here;
...
while True:
    ...
    # First container transform and rendering
    transform = glm.mat4(1.0) # identity matrix (1.0 at the diagonal)
    transform = glm.translate(transform,glm.vec3(0.5,-0.5,0.0))
    transform = glm.rotate(transform,glm.radians(pygame.time.get_ticks()/100.0),glm.vec3(0.0,0.0,1.0)) # rotate around the Z axis
    ...
    # Second container transform and rendering
    transform = glm.mat4(1.0) # identity matrix (1.0 at the diagonal)
    scale_factor = glm.sin(pygame.time.get_ticks()/1000.0)
    transform = glm.translate(transform,glm.vec3(-0.5,0.5,0.0)) 
    transform = glm.scale(transform,glm.vec3(scale_factor,scale_factor,scale_factor)) # scale up and down over time
