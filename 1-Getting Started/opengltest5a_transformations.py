...
Excercise:  Let's do something more interesting and scale and rotate the container object from the previous chapter
NOTE: for transformations we are using pyGLM but Numpy would have been good as well
'''
...
### GLM section
# matrix in GLM and OpenGL uses column-major ordering so there is no need to transpose the matrices.
# Numpy for example uses row-major ordering, so if used instead of GLM, matrices needs to be transposed
# to rotate around a vector, the vector needs first to be normalized, meaning that it has to be a UNIT vector
# (normalization extrapolates the direction from a vector)
#
# NOTE: column-major ordering means also that operations on the matrices are applied from the last to the first
#       or it an be said that are applied "from right to left"
# define the transform matrix
transform = glm.mat4(1.0) # identity matrix (1.0 at the diagonal)
transform = glm.rotate(transform,glm.radians(90.0),glm.vec3(0.0,0.0,1.0)) # rotate around the Z axis
transform = glm.scale(transform,glm.vec3(0.5,0.5,0.5)) # scale down to 50%


ptr = glm.value_ptr(transform)
transform_size = transform.length() * transform.length()
float_array = (ctypes.c_float * transform_size).from_address(ctypes.addressof(ptr.contents))
transform_bytes = bytes(float_array)

#  a)Use .value for simple uniform types (floats, ints, small vectors).
#  b)Use .write() for complex uniform types (matrices) and whenever you need precise control over the data being sent to the uniform.
#
#  Therefore, for mat4 uniforms, .write() is the recommended and most reliable method.

prog["transform"].write(transform_bytes)
