...
Excercise:  Try to display only the center pixels of the texture image on the rectangle
            in such a way that the individual pixels are getting visible by changing 
            the texture coordinates.
            Try to set the texture filtering method to GL_NEAREST to see the pixels more clearly
            NOTE: with F2 texture filtering changes from NEAREST to LINEAR
'''
...
vertices = [
    # positions          # texture coords
     0.5,  0.5, 0.0,     0.55, 0.55,   # top right
     0.5, -0.5, 0.0,     0.55, 0.45,   # bottom right
    -0.5, -0.5, 0.0,     0.45, 0.45,   # bottom left
    -0.5,  0.5, 0.0,     0.45, 0.55    # top left
]
...
texture1.filter = (moderngl.NEAREST,moderngl.NEAREST) # pylint: disable=no-member
texture2.filter = (moderngl.NEAREST,moderngl.NEAREST) # pylint: disable=no-member

## set texture 1 (the container) to clamp at the edge on both axis
texture1.repeat_x = False
texture1.repeat_y = False
    ...
    for event in pygame.event.get():
        if  event.type == pygame.QUIT:
            ...
            if event.key == pygame.K_ESCAPE:
                ...
                if texture1.filter == (moderngl.NEAREST,moderngl.NEAREST): # pylint: disable=no-member
                    texture1.filter = (moderngl.LINEAR,moderngl.LINEAR) # pylint: disable=no-member
                    texture2.filter = (moderngl.LINEAR,moderngl.LINEAR) # pylint: disable=no-member
                else:
                    texture1.filter = (moderngl.NEAREST,moderngl.NEAREST) # pylint: disable=no-member
                    texture2.filter = (moderngl.NEAREST,moderngl.NEAREST) # pylint: disable=no-member