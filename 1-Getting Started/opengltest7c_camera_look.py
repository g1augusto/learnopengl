...
### Camera vectors
# These will influence the "view" matrix that commonly is associated with the notion of a camera in OpenGL
# the vectors initialized here will be modified at the rendering loop to rotate the camera around the center of the scene
...
while True:
    # pitch constraints to simulate natural vertical camera move of an FPS
    if(pitch > 89.0):
        pitch =  89.0
    if(pitch < -89.0):
        pitch = -89.0
    ...
    # this is the scalar increment of the camera movement that will be applied at key movement
    cameraSpeed = 0.05 * NormalizedDeltaTime
    cameraTurn = 1 * NormalizedDeltaTime
    ## Model Matrices 
    # View Matrix
    ...
    # Projection Matrix
    projection = glm.perspective(glm.radians(fov),height/ width, 0.1, 100.0)
    ...
    for event in pygame.event.get():
        ...
        if event.type == pygame.MOUSEBUTTONDOWN: # when a mouse button is clicked on the window
            ...
            if pygame.event.get_grab():
                ...
                yaw += relative_x * mouseSensitivity  # Adjust sensitivity
                pitch -= relative_y * mouseSensitivity
    ...
    if keys[pygame.K_s]:
        cameraPos -= cameraSpeed * cameraTarget
    if keys[pygame.K_q]:
        pitch += cameraTurn * NormalizedDeltaTime
    if keys[pygame.K_e]:
        pitch -= cameraTurn * NormalizedDeltaTime
    if keys[pygame.K_RIGHT]:
        yaw += cameraTurn * NormalizedDeltaTime
