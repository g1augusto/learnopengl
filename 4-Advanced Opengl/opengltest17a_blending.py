...
planeVertices_binaryformat = struct.pack(f"{len(planeVertices)}f",*planeVertices)
# now for the vertices of the windows
windowsVertices_binaryformat = struct.pack(f"{len(windowsVertices)}f",*windowsVertices)


# Define VBO (Vertex Buffer Object) containing vertex data
vbo_cubeVertices = context.buffer(cubeVertices_binaryformat)

# Define VBO for the floor VAO
vbo_planeVertices = context.buffer(planeVertices_binaryformat)

# Define VBO for the windows VAO
vbo_windowsVertices = context.buffer(windowsVertices_binaryformat)
...
floorTexture  = pygame.image.load("./assets/metal.png")
windowTexture = pygame.image.load("./assets/blending_transparent_window.png")
...
floorTexture_data = pygame.image.tobytes(floorTexture,"RGBA",True)
windowTexture_data = pygame.image.tobytes(windowTexture,"RGBA",False) # Third parameter is set to NOT invert the texture since the texture coordinates are already inverted

# load the texture within the OpenGL context
cubeTexture_Texture = context.texture(cubeTexture.get_size(),4,cubeTexture_data)
floorTexture_Texture = context.texture(floorTexture.get_size(),4,floorTexture_data)
windowTexture_Texture = context.texture(windowTexture.get_size(),4,windowTexture_data)

# Set grass texture REPEAT x and y to CLAMP TO EDGE by setting it to FALSE (same configuration as SAMPLERS)
# https://moderngl.readthedocs.io/en/stable/reference/sampler.html#Sampler.repeat_x
# LearnOpengl.com notes on this:
# Note that when sampling textures at their borders, OpenGL interpolates 
# the border values with the next repeated value of the texture 
# (because we set its wrapping parameters to GL_REPEAT by default). 
# This is usually okay, but since we're using transparent values, 
# the top of the texture image gets its transparent value interpolated 
# with the bottom border's solid color value. 
# The result is then a slightly semi-transparent colored border you may 
# see wrapped around your textured quad. 
# To prevent this, set the texture wrapping method to 
# GL_CLAMP_TO_EDGE whenever you use alpha textures that you don't want to repeat:
windowTexture_Texture.repeat_x  = False
windowTexture_Texture.repeat_y = False
...
# now for the floor
vbo_planeVertices_parameters = [
    (vbo_planeVertices,"3f 2f","aPos","aTexCoords")
]

# now for windows
vbo_windowsVertices_parameters = [
    (vbo_windowsVertices,"3f 2f","aPos","aTexCoords")
]
...
vao_planeVertices = context.vertex_array(prog,vbo_planeVertices_parameters)

# VAO for the windows
vao_windowsVertices = context.vertex_array(prog,vbo_windowsVertices_parameters)
...
while True:
    ...
    ## render floor
    # bind the texture to the texture unit related to the uniform variable
    floorTexture_Texture.use(location=0)
    model:glm.mat4 = glm.mat4(1.0)
    model = glm.translate(model, glm.vec3(2.0, 0.0, 0.0))
    prog["model"].write(matrix_bytes(model))
    vao_planeVertices.render(moderngl.TRIANGLES)

    ## render windows
    # bind the texture to the texture unit related to the uniform variable
    windowTexture_Texture.use(location=0)

    # create a sorted list of distances between the camera position and the windows
    # sort it in descening order (from the farest to the nearest)
    windows.sort(key=lambda x: glm.length2(cam.cameraPos - x),reverse=True)

    for location in windows:
        model:glm.mat4 = glm.mat4(1.0)
        model = glm.translate(model, location)
        prog["model"].write(matrix_bytes(model))
        vao_windowsVertices.render(moderngl.TRIANGLES)
