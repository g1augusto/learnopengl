...
ourModel = Model(context,progReflections,modelFile)
...
while True:
    ...
    # render model
    progReflections["model"].write(matrix_bytes(model))
    progReflections["view"].write(matrix_bytes(view))
    progReflections["projection"].write(matrix_bytes(projection))
    progReflections["cameraPos"].value = cam.cameraPos
    # could work via implicit declaration same skybox uniform defined at Texture Unit 0 on both progReflections and progSkybox
    # shaders but is better to explicitly declare them
    progReflections["skybox"].value = 0 
    ourModel.Draw(progReflections)
