...
class Vertex:
    ...
    def size(self):
        #size = ( glm.sizeof(self.Position) + glm.sizeof(self.Normal) + glm.sizeof(self.TexCoords) ) // 4 # NOTE Not using texture in this chapter
        #size = ( glm.sizeof(self.Position) + glm.sizeof(self.TexCoords) ) // 4 
        size = ( glm.sizeof(self.Position) + glm.sizeof(self.Normal)) // 4
        return size  
...
class Mesh():
    ...
    def setupMesh(self):
        ...
        for vertex in self.vertices:
            #components = list(vertex.Position) + list(vertex.Normal) + list(vertex.TexCoords) # NOTE Not using texture in this chapter
            #components = list(vertex.Position) + list(vertex.TexCoords)
            components = list(vertex.Position) + list(vertex.Normal)
        ...
        # vbo_parameters = [(self.vbo,vbo_string,"aPos","aNormal","aTexCoords")] # NOTE Not using texture in this chapter
        vbo_string = f"{glm.sizeof(self.vertices[0].Position)//4}f {glm.sizeof(self.vertices[0].Normal)//4}f"
        vbo_parameters = [(self.vbo,vbo_string,"aPos","aNormal")]
...
void main()
{
    ...
    // This forces the depth value of every skybox pixel to be 1.0, the maximum possible depth.
...
progReflections = context.program(
    vertex_shader='''
#version 330 core
layout (location = 0) in vec3 aPos;
layout (location = 1) in vec3 aNormal;
...
progRefractions = context.program(
    vertex_shader='''
#version 330 core
layout (location = 0) in vec3 aPos;
layout (location = 1) in vec3 aNormal;
...
ourModel = Model(context,progRefractions,modelFile)
...
############### CUBEMAPS SECTION
# Cubemaps has 6 faces of the same size (width,height) and MUST ust be loaded in the following order
# 
...
# 
# In order to achieve this we are going to load each image in the correct sequence via pygame and 
# store each object byte data in a list containing the byte data of each image loaded (here we used just 2 list comprehensions)

cubemap_sequence = ["right","left","top","bottom","front","back"]
cubemap_images = [pygame.image.load(f"./assets/skybox/{x}.jpg") for x in cubemap_sequence]
# here we store the byte data of each image in the same ordered sequence
cubemap_images_bytes = [pygame.image.tobytes(x,"RGB",False) for x in cubemap_images]
...
cubemapTexture.repeat_x = False
cubemapTexture.repeat_y = False
cubemapTexture.repeat_z = False
...
skyboxVertices_binaryformat = struct.pack(f"{len(skyboxVertices)}f",*skyboxVertices)

skybox_vbo = context.buffer(skyboxVertices_binaryformat)

skybox_vbo_parameters = [
    (skybox_vbo,"3f","aPos")
]

skybox_vao = context.vertex_array(progSkybox,skybox_vbo_parameters)
...
while True:
    message = f"-- Refraction: {refraction_table[sourceMaterial]["mat"]} to {refraction_table[destMaterial]["mat"]} (press 0..4 / SHIFT to change)"
    ...
    # render model
    progRefractions["model"].write(matrix_bytes(model))
    progRefractions["view"].write(matrix_bytes(view))
    progRefractions["projection"].write(matrix_bytes(projection))
    progRefractions["cameraPos"].value = cam.cameraPos
    progRefractions["skybox"].value = 0 
    progRefractions["ratio"].value = refraction_table[sourceMaterial]["index"] / refraction_table[destMaterial]["index"]
    ourModel.Draw(progRefractions)
    ...
    # This way the skybox will always be drawn at the background of all the other objects since the unit cube is most likely smaller than the rest of the scene

    # Why disable DEPTH MASK
    # If you draw the skybox last, you run into a problem. Your depth buffer is already filled with the depth values of your scene. 
    # When you try to draw the skybox, its geometry is just a simple cube around the camera, so it's technically very "close." 
    # The depth test would see that the skybox pixels are closer than the distant mountains already in the depth buffer, 
    # and it would incorrectly draw the skybox in front of everything.

    
    # Change the depth function to LESS EQUAL
    # This changes how the depth test works. The default is '<' (LESS), 
    # meaning a new pixel is only drawn if it's strictly closer than what's already on screen. We change it to '<=' (LESS THAN OR EQUAL).
    # vertex shader uses the gl_Position = pos.xyww; trick. 
    # This forces the depth value of every skybox pixel to be 1.0, the maximum possible depth. 
    # The screen is also cleared to a depth of 1.0 at the start of the frame. 
    # The default '<' test would fail because 1.0 is not less than 1.0. By changing the test to '<= ', 
    # we allow the skybox to be drawn over the "empty" parts of the background.
    context.depth_func = "<="
    # remove depth writing from the current active framebuffer
    # This tells the GPU: "You can draw the colors of the skybox pixels, but do not update the depth buffer with their depth value."
    # This is a crucial optimization. Since the skybox is drawn last, 
    # the depth buffer already contains the correct depth information for your entire scene (your models, etc.). 
    # If we allowed the skybox to write its depth value (1.0), it would erase all that precious information, messing up rendering in the next frame.
    # We draw the skybox for its color but leave the depth buffer untouched.
    context.screen.depth_mask = False
    cubemapTexture.use(location=0)
    # This is the clever trick to make the skybox seem infinite. 
    # It takes your camera's normal view matrix but throws away the translation part (the part that says where the camera is). 
    # The result is a matrix that only contains the camera's rotation. 
    # The skybox will now turn with you, but you can never move closer to or farther from it.
    viewSkybox = glm.mat4(glm.mat3(view))
    progSkybox["view"].write(matrix_bytes(viewSkybox))
    progSkybox["projection"].write(matrix_bytes(projection))
    progSkybox["skybox"].value = 0 # Use the integer of the texture unit
    skybox_vao.render()
    # re-enable depth writing
    context.screen.depth_mask = True
    context.depth_func = "<" # Reset to the default depth function
