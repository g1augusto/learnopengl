...
# now for the vertices of the vegetation
vegetationVertices_binaryformat = struct.pack(f"{len(vegetationVertices)}f",*vegetationVertices)
...
# Define VBO for the vegetation VAO
vbo_vegetationVertices = context.buffer(vegetationVertices_binaryformat)
...
grassTexture_data = pygame.image.tobytes(grassTexture,"RGBA",False) # Third parameter is set to NOT invert the texture since the texture coordinates are already inverted
...
grassTexture_Texture = context.texture(grassTexture.get_size(),4,grassTexture_data)
...
grassTexture_Texture.repeat_x  = False
grassTexture_Texture.repeat_y = False
...
# now for vegetation
vbo_vegetationVertices_parameters = [
    (vbo_vegetationVertices,"3f 2f","aPos","aTexCoords")
]
...
# VAO for the vegetation
vao_vegetationVertices = context.vertex_array(prog,vbo_vegetationVertices_parameters)
...
while True:
    ...
    ## render vegetation
    # bind the texture to the texture unit related to the uniform variable
    grassTexture_Texture.use(location=0)
    # Set OpenGL Blending 
    #context.enable(moderngl.BLEND) # GEMINI
    #context.blend_func = (moderngl.SRC_ALPHA,moderngl.ONE)
    for location in vegetation:
        model:glm.mat4 = glm.mat4(1.0)
        model = glm.translate(model, location)
        prog["model"].write(matrix_bytes(model))
        vao_vegetationVertices.render(moderngl.TRIANGLES)
    # reset openGL blending to default (disabled)
