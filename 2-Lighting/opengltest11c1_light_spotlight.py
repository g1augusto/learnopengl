...
Excercise:  Spotlight
            ...
            The result is that only the objects within a certain radius of the spotlight's direction are
            lit and everything else stays dark. A good example of a spotlight would be a street lamp or a flashlight.
...
while True:
    # pass spotlight positional and cone data
    prog["light.position"] = cam.cameraPos
    prog["light.direction"] = cam.cameraTarget
    prog["light.cutOff"] = glm.cos(glm.radians(cutOffAngle))
    prog["light.outerCutOff"] = glm.cos(glm.radians(outerCutOffAngle))
    ...
    prog["light.constant"] = lightConstant
    prog["light.linear"] = lightLinear
    prog["light.quadratic"] = lightQuadratic

    # Here we update the model matrix for the cubes
