...
Excercise:  Diffuse lighting
            ...
            NOTE: Normal Vectors (used to calculate the angle of incidence of the light source to the surface) 
                  from the previous example should have also taken into consideration world transforms (that were not applied in that example).
                  The way to apply world transforms to normal Vectors is to:
                        multiply each normal vector for the "normal matrix"
                  "Normal Matrix" is calculated as 'the transpose of the inverse of the upper-left 3x3 part of the model matrix'.
                  In this example we are going to calculate it in the VERTEX SHADER but is suboptimal and inefficient
                  since we are going to recalculate the normal matrix for EACH VERTEX!
                  In the next example we are going to calculate it on the CPU and pass it via an uniform 
'''
...
void main()
{
    ...
    Normal = mat3(transpose(inverse(model))) * aNormal; // Here we calculate the Normal value using the Normal Matrix
