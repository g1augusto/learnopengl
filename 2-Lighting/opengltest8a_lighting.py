...
Excercise:  A lighting scene
            The first thing we need is an object to cast the light on and we'll use the infamous container cube
            from the previous chapters. We'll also be needing a light object to show where the light source is
            located in the 3D scene. 
...
void main()
{
    //Here we multiply the light's color with the object's
...
lightvao = context.vertex_array(progLight,vbo_parameters)
