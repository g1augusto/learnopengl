...
Excercise:  Specular Lighting
            ...
            NOTE: We chose to do the lighting calculations in world space, but most people tend to prefer doing lighting in view space.
                  An advantage of view space is that the viewer's position is always at (0,0,0) so you already got the position of the
                  viewer for free. However, I find calculating lighting in world space more intuitive for learning purposes.
                  If you still want to calculate lighting in view space you want to transform all the relevant vectors with the
                  view matrix as well (don't forget to change the normal matrix too).

            Excercise:  Do Phong shading in view space instead of world space
                        
                        ------------------------------------
                        F2: start/stop spinning light
                        F3: increase ambient light
                        F4: decrease ambient light
                        F5: increase diffuse light
                        F6: decrease diffuse light
                        F7: increase shininess
                        F8: decrease shininess
'''
...
out vec3 lightPosition; // variable that will be sent and used in the fragment shader
...
void main()
{
    ...
    FragPos = vec3(view * model * vec4(aPos,1.0)); // Fragment position in the world calculated by the model*view matrix (before we did it only for the model matrix)
...
uniform float ambientStrength; // input value for ambient light strenght
uniform float specularStrength; // specular intensity of the surface fixed at a certain value
uniform int shininess; // how much the specular light shines
void main()
{   
    // Ambient Lighting
    ...
    // Specular lighting
    vec3 viewDirection = normalize(-FragPos); // the viewer is always at (0,0,0) in view-space, so viewDir is (0,0,0) - Position => -Position
    ...
    vec3 result = (ambient + diffuseComponent + specular) * objectColor;
...
while True:
    ...
    # NOTE: In this case we are calculating the normal matrix in the VIEW SPACE (before we did it in the WORLD SPACE)
    #       and the transpose of the inverse needs to be taken from the view*model matrix
    ...
    prog["ambientStrength"] = ambientStrength
    prog["specularStrength"] = specularStrength
