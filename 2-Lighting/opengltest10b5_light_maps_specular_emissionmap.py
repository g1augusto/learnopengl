...
Excercise:  Specular maps
            The intensity of the specular highlight comes from the brightness of each pixel in the image. 
            Each pixel of the specular map can be displayed as a color vector where black represents the
            color vector vec3(0.0) and gray the color vector vec3(0.5) for example.
            In the fragment shader we then sample the corresponding color value and multiply this value
            with the light's specular intensity.
            The more 'white' a pixel is, the higher the result of the multiplication and thus the
            brighter the specular component of an object becomes.
            NOTE: At this point we have a light system that is composed of the following:
                    - Light source parameters 
                        -   ambient strenght
                        -   diffuse strenght
                        -   specular strenghts
                    - Material properties
                        -   Texture for diffuse and ambient color
                        -   Texture for specular color
                        -   shininess (as a factor 2 value of 2/8/16/32/64/128/256)

            Excercise:  Also add something they call an emission map which is a texture that stores emission
                        values per fragment. Emission values are colors an object may emit as if it contains a
                        light source itself; this way an object can glow regardless of the light conditions.
                        Emission maps are often what you see when objects in a game glow (like eyes of a robot,
                        or light strips on a container)
...
struct Material {
    ...
    sampler2D emission; // emission map to add to the light
    float     shininess;
}; 
...
{   
    ...
    vec3 specular = light.specular * spec * vec3(texture(material.specular,TexCoords)); // we are inverting here by subtracting vec3(1.0) by the actual specular map

    // calculate the emission map to add to the fragment final color
    // the idea is that it is independent of any light source
    ...
    vec3 result = ambient + diffuseComponent + specular + emission;
...
emissionMapimage = pygame.image.load("./assets/matrix.jpg") 
...
emissionMapimage_data = pygame.image.tobytes(emissionMapimage,"RGBA",True)
...
emissionMapTexture = context.texture(emissionMapimage.get_size(),4,emissionMapimage_data)
...
emissionMapTexture.use(location=2)
...
while True:
    ...
    if moveLight:
        radius = 2.0
        lightX = math.sin(pygame.time.get_ticks()/1000) * radius
        lightZ = math.cos(pygame.time.get_ticks()/1000) * radius
        lightY = lightPos.y
        # Now pass the value of the light position vector as uniform
        lightPos = glm.vec3(lightX,lightY,lightZ)
    prog["light.position"] = lightPos
