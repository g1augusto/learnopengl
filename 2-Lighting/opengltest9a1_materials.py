...
Excercise:  Specular Lighting
            ...
            Excercise:  Play around with different ambient, diffuse and specular strengths and see how they impact the result. 
...
''',
    ...
    fragment_shader='''
#version 330 core
struct Material { // material data type
    vec3 ambient;
    vec3 diffuse;
    vec3 specular;
    float shininess;
...
{   
    // Ambient Lighting
    vec3 ambient = light.ambient * material.ambient; // light color is no more used
    ...
    vec3 diffuseComponent = light.diffuse * (diffuseImpact * material.diffuse); // light color is no more used
    
    // Specular lighting
    vec3 viewDirection = normalize(viewPos - FragPos);
    vec3 reflectDirection = reflect(-lightDirection, normalized); 
    float spec = pow(max(dot(viewDirection, reflectDirection), 0.0), material.shininess);
    vec3 specular = light.specular * (spec * material.specular); // light color is no more used
...
# Material parameters
prog["material.ambient"] = glm.vec3(1.0,0.5,0.31)
prog["material.diffuse"] = glm.vec3(1.0,0.5,0.31)
prog["material.specular"] = glm.vec3(0.5,0.5,0.5)
prog["material.shininess"] = 32.0

# Light source parameters
prog["light.ambient"] = glm.vec3(0.2, 0.2, 0.2)
prog["light.diffuse"] = glm.vec3(0.5, 0.5, 0.5)
prog["light.specular"] = glm.vec3(1.0, 1.0, 1.0)
